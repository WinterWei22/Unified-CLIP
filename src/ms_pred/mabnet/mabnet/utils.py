...
class NeighborEmbedding(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_weight, edge_attr):
        ...
        x_neighbors = self.propagate(edge_index, x=x, W=W, size=None)
...
class TwoBodyAttn(nn.Module):
    ...
    def forward(self, feat):
        ...
        Va_in = torch.einsum('ijh, jdh->idh', A_in, V_in)     # torch.Size([num_nodes, head_dim, num_heads])
        Va_in = Va_in.flatten(start_dim=1)  # torch.Size([num_nodes, head_dim * num_heads])
...
class ManyBodyAttn(nn.Module):
    ...
    def forward(self, feat1, feat2):
        ...
        Va_in = torch.einsum('ijkh, jkdh->ijdh', A_in, V_in)     # torch.Size([num_nodes, num_nodes, head_dim, num_heads])
        Va_in = Va_in.flatten(start_dim=2)  # torch.Size([num_nodes, num_nodes, head_dim * num_heads])
...
class TwoBodyPadAttn(nn.Module):
    ...
    def forward(self, feat, mask=None):
        ...
        if mask is not None:    # mask.shape: [batchsize, num_nodes, num_nodes]
        ...
        gates_in = torch.sigmoid(G_in)
        A_in = torch.softmax(H_in, dim=0) * gates_in
        Va_in = torch.einsum('bijh, bjdh->bidh', A_in, V_in)     # torch.Size([batch_size, num_nodes, head_dim, num_heads])
        Va_in = Va_in.flatten(start_dim=2)  # torch.Size([batch_size, num_nodes, head_dim * num_heads])
...
class ManyBodyPadAttn(nn.Module):
    ...
    def forward(self, feat1, feat2, mask=None):
        ...
        # A_in = torch.softmax(H_in, dim=0) * gates_in
        # A_in = torch.softmax(H_in, dim=0)
        Va_in = torch.einsum('bijkh, bjkdh->bijdh', A_in, V_in)     # torch.Size([batch_size, num_nodes, num_nodes, head_dim, num_heads])
        Va_in = Va_in.flatten(start_dim=3)  # torch.Size([batch_size, num_nodes, num_nodes, head_dim * num_heads])
