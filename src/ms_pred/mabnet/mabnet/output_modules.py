...
from ms_pred.mabnet.mabnet.utils import act_class_mapping
...
class EquivariantScalarWithUncertainty(OutputModel):
    def __init__(self, hidden_channels, activation="silu", allow_prior_model=True, uncertainty_model=None):
        super(EquivariantScalar, self).__init__(allow_prior_model=allow_prior_model)
    ...
    def reset_parameters(self):
        for layer in self.output_network:
            layer.reset_parameters()
        for layer in self.uncertainty_model:
            layer.reset_parameters()
