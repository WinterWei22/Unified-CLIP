...
from ms_pred.clip.clip_model import Mol_Encoder_with_adducts

class MLP(nn.Module):
    def __init__(self, hidden_dim: int, intermediate_dim: int = None):
        super(MLP, self).__init__()
        # 如果未指定中间维度，默认设置为 hidden_dim * 2
        ...
        self.mlp = nn.Sequential(
            nn.Linear(input_dim, intermediate_dim),  # 输入层到隐藏层
            nn.ReLU(),                              # 非线性激活
            nn.Linear(intermediate_dim, hidden_dim)  # 隐藏层到输出层
        )
    
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x 的形状应为 [bs, hidden_dim + 10]
        return self.mlp(x)  # 输出形状 [bs, hidden_dim]
...
class IntenGNN(pl.LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        if self.encode_forms:
            ...
            self.formula_dim = common.NORM_VEC.shape[0]  # 18
        ...
        self.clip_from_adducts = clip_from_adducts
        self.root_only = root_only  # Save new parameter
        
        self.embed_instrumentation = embed_instrumentation
        ...
        if self.clip_from_adducts:
            self.gnn = Mol_Encoder_with_adducts(       
                                                input_node_dim=input_node_dim,
                                                hidden_size=hidden_size,
                                                mabnet_heads=frag_heads,
                                                mabnet_layers=frag_layers,
            ...
            self.gnn = egt_model.EdgeEnhancedGraphTransformer2D(input_node_dim=input_node_dim,
                                                                hidden_dim=hidden_size,
                                                                num_heads=frag_heads,
                                                                num_layers=frag_layers,
                                                                edge_update=edge_update,
                                                                )
        self.root_adduct_layer = MLP(hidden_dim=hidden_size, intermediate_dim=hidden_size)
        self.frag_adduct_layer = MLP(hidden_dim=hidden_size, intermediate_dim=hidden_size)
        # self.gnn = PyGGNN(
        #     hidden_size=self.hidden_size,
        #     num_layers=self.gnn_layers,
        #     node_feats=node_feats + adduct_shift,
        ...
        if self.root_encode == "gnn":
            ...
            self.root_module = egt_model.EdgeEnhancedGraphTransformer3D(hidden_dim=hidden_size,
                                                                      num_heads=mabnet_heads,
                                                                      num_layers=mabnet_layers,
                                                                      edge_update=edge_update,
                                                                      )
            ...
            if self.clip_from_adducts:
                self.root_module = Mol_Encoder_with_adducts(       
                                                    input_node_dim=input_node_dim,
                                                    hidden_size=hidden_size,
        ...
        inter_size = self.hidden_size * 3 + self.max_broken + self.formula_in_dim
        # if self.embed_instrumentation:
        #     inter_size += num_instrument_types
        ...
        buckets = torch.DoubleTensor(np.linspace(0, 1500, 15000))   # (0, 1500, 15000)
        ...
        if self.cross_attention_mode:
            self.cross_attn = nn.MultiheadAttention(
                embed_dim=self.hidden_size,  # 假设dim == hidden_size
                num_heads=4,
                dropout=0.1,
                batch_first=True
            )
            self.gate_mlp = nn.Sequential(
                ...
                nn.Sigmoid()
            )  # 用于gated fusion of differences
            
        if self.root_only:
            self.root_only_predictor = nn.Linear(self.hidden_size, self.inten_buckets.shape[0])
    ...
    def forward(
        ...
        if self.root_encode == "fp":
            ...
            if self.embed_instrumentation:
                ...
                root_repr.h = root_h
                _, root_embeddings = self.root_module(root_repr)
        ...
        elif self.root_encode in ['egt', 'egt2d', 'egt3d']:
            if self.clip_from_adducts:
                _, root_embeddings = self.root_module(root_repr, adducts.long())
            elif self.embed_adduct or self.embed_instrumentation:
                root_h = root_repr.h
                if self.embed_adduct:
                    root_batch_sizes = torch.bincount(root_repr.batch) if root_repr.batch is not None else torch.tensor([root_repr.num_nodes])
                    embed_adducts_expand = embed_adducts.repeat_interleave(root_batch_sizes, 0)  # (num_nodes, 10)
                    root_h = torch.cat([root_repr.h, embed_adducts_expand], -1)  # (num_nodes, 42+10)
                if self.embed_instrumentation:
                    embed_instruments = self.instrument_embedder[instruments.long()] 
                    embed_instruments_expand = embed_instruments.repeat_interleave(root_batch_sizes, 0)
                    root_h = torch.cat([root_h, embed_instruments_expand], -1)
            ...
            _, root_embeddings = self.root_module(root_repr)
            root_embeddings = self.root_adduct_layer(torch.cat([root_embeddings, embed_adducts], -1))
        ...
        if self.root_only:
            # Directly predict binned spectrum from root_embeddings
            output_binned_raw = self.root_only_predictor(root_embeddings)  # (bs, 15000)
            output_binned = self.sigmoid(output_binned_raw).unsqueeze(1)  # (bs, 1, 15000)
            return {"output_binned": output_binned, "output": None}
        
        if self.embed_adduct:
            if self.clip_from_adducts:
                # adducts_mapped = embed_adducts[ind_maps]
                _, avg_frags = self.gnn(graphs, adducts.long(), ind_maps)  # (f_nodes, hidden_size)
                # avg_frags = self.frag_adduct_layer(torch.cat([avg_frags, adducts_mapped], -1))
                # adducts_mapped = embed_adducts[ind_maps]
                # adducts_exp = torch.repeat_interleave(adducts_mapped, graph_batch_sizes, dim=0)
                # graphs_h = aggregate_edge_features(graphs)
                # graphs_h = torch.cat([graphs_h, adducts_exp], -1).float()
                # graphs.h = graphs_h
                # avg_frags = self.gnn(graphs_h, graphs.edge_index)  # (f_nodes, hidden_size)
                # avg_frags = self.pool(avg_frags, graphs.batch)  # (n_frags, hidden_size)
            elif self.embed_adduct or self.embed_instrumentation:
                graphs_h = graphs.h
                if self.embed_adduct:
                    adducts_mapped = embed_adducts[ind_maps]
                    graph_batch_sizes = torch.bincount(graphs.batch) if graphs.batch is not None else torch.tensor([graphs.num_nodes])
                    adducts_mapped_expand = adducts_mapped.repeat_interleave(graph_batch_sizes, 0)  # (num_nodes, 10)
                    graphs_h = torch.cat([graphs_h, adducts_mapped_expand], -1)  # (num_nodes, 42+10)
                if self.embed_instrumentation:
                    embed_instruments = self.instrument_embedder[instruments.long()] 
                    instruments_mapped = embed_instruments[ind_maps]
                    instruments_mapped_expand = instruments_mapped.repeat_interleave(graph_batch_sizes, 0)
                    graphs_h = torch.cat([graphs_h, instruments_mapped_expand], -1)
                graphs.h = graphs_h
                _, avg_frags = self.gnn(graphs)
            else:
                _, avg_frags = self.root_module(graphs)
                avg_frags = self.root_adduct_layer(torch.cat([avg_frags, embed_adducts], -1))
        ...
        # 显式定义 max_frags
        max_frags = broken.shape[-1]

        # 早期融合：保留gated_diff，但去除早期cross-attention
        if self.cross_attention_mode:
            # Gated fusion for differences (保留作为基本交互)
            diff = ext_root - avg_frags  # (frags, dim)
            gate = self.gate_mlp(torch.cat([ext_root, diff], dim=-1))  # (frags, dim)
            gated_diff = gate * diff  # (frags, dim)
            mlp_cat_list = [ext_root, gated_diff, avg_frags, broken_onehots]  # 使用avg_frags代替fused (frags, dim*3 + max_broken+1)
        ...
        if self.encode_forms:
            ...
            padded_hidden = torch.cat([padded_hidden, form_encodings, diff_encodings], dim=-1)  # (bs, max_frags, hidden_size*3+max_broken+1+288)
        ...
        # 新增：移动cross-attention到Transformer之后（后期融合）
        if self.cross_attention_mode:
            # 扩展分子embedding到序列（作为key/value）
            ext_root_padded = root_embeddings[:, None, :].expand(-1, max_frags, -1)  # (bs, max_frags, hidden_size) - 重复分子到每个碎片

            # Cross-attention: hidden (Transformer输出) as query, ext_root_padded as key/value
            fused_hidden, cross_attn_weights = self.cross_attn(
                query=hidden,  # (bs, max_frags, hidden_size)
                key=ext_root_padded,
                value=ext_root_padded,
                key_padding_mask=attn_mask  # 忽略padding碎片
            )  # fused_hidden: (bs, max_frags, hidden_size)

            # 残差连接以稳定
            hidden = hidden + fused_hidden  # (bs, max_frags, hidden_size)
    ...
    def _common_step(self, batch, name="train"):
        pred_obj = self.forward(
            ...
            instruments=batch["instruments"] if self.embed_instrumentation else None,
