...
import ms_pred.mabnet.gen_pyg_model as gen_model
...
def get_args():
    ...
    parser.add_argument("--batch-size", default=0, action="store", type=int)
...
def predict():
    ...
    model = gen_model.FragEGT.load_from_checkpoint(best_checkpoint, map_location="cpu")
    ...
    with torch.no_grad():
        ...
        def single_predict_mol(entry):
            ...
            instrument = entry["instrumentation"] if "instrumentation" in entry else "unknown"
            # instrument = common.CANOPUS_INSTRUMENTATION_ONEHOT.get(instrument, 0)
