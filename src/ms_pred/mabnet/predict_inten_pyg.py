...
from ms_pred.mabnet import dag_pyg_data, inten_mabnet_model
...
def predict():
    ...
    model = inten_mabnet_model.IntenGNN.load_from_checkpoint(best_checkpoint)
    ...
    pred_dataset = dag_pyg_data.IntenPredDataset(
        df,
        tree_processor=tree_processor,
        num_workers=num_workers,
        data_dir=data_dir,
        magma_map=name_to_json,
        embed_instrumentation=True
    )
    # Define dataloaders
    ...
    with torch.no_grad():
        for batch in tqdm(pred_loader):
            ...
            instruments = batch["instruments"].to(device) if "instruments" in batch else None
