...
from ms_pred.mabnet import dag_pyg_data, gen_pyg_model
...
def add_frag_train_args(parser):
    ...
    parser.add_argument("--num-gpu", default=1, type=int, action="store")
...
def train_model():
    ...
    train_dataset = dag_pyg_data.GenDataset(
        train_df,
        data_dir=data_dir,
    ...
    val_dataset = dag_pyg_data.GenDataset(
        val_df,
        data_dir=data_dir,
        magma_map=name_to_json,
        num_workers=num_workers,
    ...
    test_dataset = dag_pyg_data.GenDataset(
        test_df,
        data_dir=data_dir,
        magma_map=name_to_json,
        tree_processor=tree_processor,
    ...
    # input_node_dim = input_node_dim = 41 + args.pe_embed_k + 10
    input_node_dim = 24 + args.pe_embed_k + 10 + len(common.CANOPUS_INSTRUMENTATION_ONEHOT) if kwargs['embed_instrumentation'] else 24 + args.pe_embed_k + 10

    # Define model
    model = gen_pyg_model.FragEGT(
    ...
    if kwargs["num_gpu"] > 1:
        ...
        if torch.distributed.get_rank() == 0:
            ...
            model = gen_pyg_model.FragEGT.load_from_checkpoint(best_checkpoint)
    else:
