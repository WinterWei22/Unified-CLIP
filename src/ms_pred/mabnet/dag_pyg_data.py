...
class TreeProcessor:
    ...
    def __init__(
        ...
        self.bins = np.linspace(0, 1500, 15000) # 1500 15000
...
class DAGDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.magma_map = magma_map  # name - tree_file_path
        self.embed_instrumentation = embed_instrumentation
        
        valid_specs = [i in self.magma_map for i in self.df["spec"].values] # spec with map
        ...
        if len(self.df_sub) == 0:
            ...
            self.conf = self.df_sub["conformer"].values.tolist()
        ...
        read_fn = lambda x: self.read_tree(self.load_tree(x))   # define tree load and read
        ...
        if embed_instrumentation:
            self.name_to_instrument = dict(self.df[["spec", "instrumentation"]].values)
            self.instrumentation = [common.CANOPUS_INSTRUMENTATION_ONEHOT.get(self.name_to_instrument[i], 0) for i in self.spec_names]
    ...
    def __getitem__(self, idx: int):
        ...
        if self.embed_instrumentation:
            instrumentation = self.instrumentation[idx]

        dgl_entry = self.read_fn(name)["dgl_tree"]
        # dgl_entry = self.dgl_trees[idx]

        outdict = {"name": name, "adduct": adduct, "instrumentation": instrumentation} if self.instrumentation else {"name": name, "adduct": adduct}

        # Convert this into a list of graphs with a list of targets
        outdict.update(dgl_entry)
        outdict['root_repr'].graph_data = {'pos': torch.from_numpy(self.conf[idx].astype(np.float32))}
        return outdict
...
class GenDataset(DAGDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            ...
            use_ray=use_ray,
            embed_instrumentation=embed_instrumentation,
        )
    ...
    def collate_fn(input_list):
        ...
        if "instrumentation" in input_list[0] and input_list[0]["instrumentation"] is not None:
            instrumentations = [i["instrumentation"] for i in input_list]
            instrumentations = torch.FloatTensor(instrumentations)
        ...
        output = {
            ...
            "frag_graphs": dgl_to_pyg(frag_batch),
