...
class PretrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # logD (here directly take cx_logp)
            logd = row["cx_logp"]
    ...
    def collate_fn(
        ...
        # Handle cases where masked_edge_labels might be None by providing empty tensors
        masked_edge_labels_list = [
            j["masked_edge_labels"] if j["masked_edge_labels"] is not None else torch.tensor([], dtype=torch.long)
            for j in input_list
        ]
        masked_edge_labels_padded = torch.nn.utils.rnn.pad_sequence(
            masked_edge_labels_list, batch_first=True, padding_value=-1
        )
        
        masked_src_list = [
            j["masked_src"] if j["masked_src"] is not None else torch.tensor([], dtype=torch.long)
            for j in input_list
        ]
        masked_src_padded = torch.nn.utils.rnn.pad_sequence(
            masked_src_list, batch_first=True, padding_value=-1
        )
        
        masked_dst_list = [
            j["masked_dst"] if j["masked_dst"] is not None else torch.tensor([], dtype=torch.long)
            for j in input_list
        ]
        masked_dst_padded = torch.nn.utils.rnn.pad_sequence(
            masked_dst_list, batch_first=True, padding_value=-1
        )
