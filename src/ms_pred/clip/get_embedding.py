...
def get_args():
    ...
    parser.add_argument(
        ...
        default="none",
        action="store",
        choices=["none", "train_only", "test_only", "debug_special", "valid_only"],
    )
...
def predict():
    ...
    if kwargs["subset_datasets"] != "none":
        ...
        elif kwargs["subset_datasets"] == "valid_only":
            names = splits[splits[fold_name] == "val"]["spec"].tolist()
    ...
    with torch.no_grad():
        for batch in tqdm(pred_loader):
            ...
            global_sims, local_sims, specs_global_proj, mol_proj = model.predict(
                specs=specs,
            ...
            for spec, spec_global_emb, mol_global_emb in zip(
                    spec_names, specs_global_proj, mol_proj
                ):
                    output_obj = {
                        "spec_name": spec,
                        "spec_global_emb": spec_global_emb,
                        "mol_global_emb": mol_global_emb
                    }
    ...
    if binned_out:
        ...
        output = {
            str(item["spec_name"]): (item["spec_global_emb"].cpu().numpy(), item["mol_global_emb"].cpu().numpy())
            for item in pred_list
        }
        
        # print(f"Mol embeddings of {kwargs['dataset_labels']} ")
        out_file = Path(kwargs["save_dir"]) / "train-spec-mol-embeddings.pkl"
