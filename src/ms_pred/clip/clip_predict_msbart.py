...
def predict():
    ...
    with torch.no_grad():
        for batch in tqdm(pred_loader):
            ...
            smiless = batch["smiless"]

            global_sims, local_sims, specs_global_proj, mol_proj, _ = model.predict(
                specs=specs,
            ...
            for spec, spec_global_emb, mol_emb, global_sim, local_sim, flag, smiles in zip(
                    spec_names, specs_global_proj, mol_proj, global_sims, local_sims, flags, smiless
                ):
                    output_obj = {
                        "spec_name": spec,
    ...
    if binned_out:
        ...
        flag_ar = [str(i["flag"]) for i in pred_list]
        ...
        print(f"{len(pred_loader)} embedding files of {kwargs['dataset_labels']} saved in {out_file}")
    else:
