"""first version of cross attention."""
import numpy as np
import copy
import torch
...
class IntenGNN(pl.LightningModule):
    ...
    def forward(
        ...
        # graphs.h = graphs_h
        _, avg_frags = self.gnn(graphs)  # (f_nodes, hidden_size)
        if self.embed_adduct:
            adducts_mapped = embed_adducts[ind_maps]
        ...
        if self.cross_attention_mode:
            # 将碎片作为序列，分子作为query
            # 显式定义 max_frags（从 padded_frags.shape 获取，或从 broken.shape[-1]）
            padded_frags = nn_utils.pad_packed_tensor(avg_frags, num_frags, 0)  # (bs, max_frags, dim)
            _, max_frags, _ = padded_frags.shape  # 或 max_frags = broken.shape[-1] 如果更合适
            attn_mask = ~(torch.arange(max_frags, device=device)[None, :] < num_frags[:, None])  # (bs, max_frags), padding mask

            # Cross-attention: root_embeddings as query (bs, 1, dim), padded_frags as key/value (bs, max_frags, dim)
            root_query = root_embeddings.unsqueeze(1)  # (bs, 1, dim)
            fused_frags, attn_weights = self.cross_attn(
                query=root_query,
                key=padded_frags,
                value=padded_frags,
                key_padding_mask=attn_mask
            )  # fused_frags: (bs, 1, dim) - 注意：这融合了碎片到分子，但我们需要per-fragment
            # 为了per-fragment融合，反转：碎片 as query, 分子 as key/value
            # 扩展分子到per-fragment
            ext_root_padded = root_embeddings[:, None, :].expand(-1, max_frags, -1)  # (bs, max_frags, dim)
            fused_frags, cross_attn_weights = self.cross_attn(
                query=padded_frags,  # (bs, max_frags, dim)
                key=ext_root_padded,
                value=ext_root_padded,
                key_padding_mask=attn_mask  # 注意：key是扩展的分子，无需mask，但统一使用
            )  # fused_frags: (bs, max_frags, dim)
            # 残差连接
            fused_frags = fused_frags + padded_frags  # (bs, max_frags, dim)
            # 解包回packed形式
            fused_frags_packed = fused_frags[broken_mask]  # (frags, dim), 使用现有broken_mask假设兼容

            # 新增：Gated fusion for differences
            diff = ext_root - avg_frags  # (frags, dim), 原始差异
            gate = self.gate_mlp(torch.cat([ext_root, diff], dim=-1))  # (frags, dim)
            gated_diff = gate * diff  # (frags, dim)
            mlp_cat_list = [ext_root, gated_diff, fused_frags_packed, broken_onehots]  # (frags, dim*3 + max_broken+1)
