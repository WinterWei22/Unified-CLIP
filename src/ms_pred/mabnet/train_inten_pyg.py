...
def add_frag_train_args(parser):
    ...
    parser.add_argument(
        ...
        default="gnn",
        action="store",
        choices=["gnn", "fp", "mabnet", "visnet", "egt2d", "egt3d", "egt"],
    ...
    parser.add_argument("--clip-ckpt", default=None, type=str, action="store")
    parser.add_argument("--encoder-ckpt", default=None, type=str, action="store")
    parser.add_argument("--frag-ckpt", default=None, type=str, action="store")

    ### MabNet parser ###   
    parser.add_argument("--mabnet-layers", default=4, type=int, action="store")
    parser.add_argument("--mabnet-heads", default=4, type=int, action="store")
    parser.add_argument("--many-body", default=False, action="store_true")
    parser.add_argument("--edge-update", default=False, action="store_true")
    parser.add_argument("--frag-layers", default=8, type=int, action="store")
    parser.add_argument("--frag-heads", default=4, type=int, action="store")
    parser.add_argument("--cross-attention-mode", default=False, action="store_true")
    parser.add_argument("--embed-instrumentation", default=False, action="store_true")
    parser.add_argument("--root-only", default=False, action="store_true")
...
def train_model():
    ...
    tree_processor = dag_pyg_data.TreeProcessor(
    ...
    train_dataset = dag_pyg_data.IntenDataset(
        train_df,
        data_dir=data_dir,
        magma_map=name_to_json,
        num_workers=num_workers,
        tree_processor=tree_processor,
        embed_instrumentation=kwargs["embed_instrumentation"],
    )
    val_dataset = dag_pyg_data.IntenDataset(
        val_df,
        data_dir=data_dir,
        magma_map=name_to_json,
        num_workers=num_workers,
    ...
    test_dataset = dag_pyg_data.IntenDataset(
        test_df,
        data_dir=data_dir,
        magma_map=name_to_json,
        num_workers=num_workers,
    ...
    clip_from_adducts = True
    input_node_dim = 26 + args.pe_embed_k + 10 + len(common.CANOPUS_INSTRUMENTATION_ELEMENTS) if kwargs["embed_instrumentation"] else 26 + args.pe_embed_k + 10
    if kwargs["encoder_ckpt"] != None:
        input_node_dim = input_node_dim
    elif kwargs["clip_ckpt"] != None:
        # clip_from_adducts = True
        # input_node_dim = input_node_dim + 10  # add adduct embedding dim
        input_node_dim = input_node_dim
    ...
    model = inten_model.IntenGNN(
        ...
        many_body=kwargs['many_body'],
        edge_update=kwargs['edge_update'],
        frag_heads=kwargs['frag_heads'],
        frag_layers=kwargs['frag_layers'],
        input_node_dim=input_node_dim,
        cross_attention_mode=kwargs['cross_attention_mode'],
        clip_from_adducts=clip_from_adducts,
        root_only=kwargs['root_only'],
        embed_instrumentation=kwargs['embed_instrumentation'],
    )


    if kwargs["encoder_ckpt"] != None:
        print(f"loading encoder ckpt from {kwargs['encoder_ckpt']}")
        pretrained_model = pretrain_model.Pretrain_EGT.load_from_checkpoint(kwargs["encoder_ckpt"], strict=False)
        model.root_module.load_state_dict(pretrained_model.root_module.state_dict(), strict=False)
        # model.gnn.load_state_dict(pretrained_model.root_module.state_dict(), strict=False)
    if kwargs["frag_ckpt"] != None:
        print(f"loading frag ckpt from {kwargs['frag_ckpt']}")
        frag_ckpt = gen_pyg_model.FragEGT.load_from_checkpoint(kwargs["frag_ckpt"], strict=False)
        model.gnn.load_state_dict(frag_ckpt.root_module.state_dict(), strict=True)
    elif kwargs["clip_ckpt"] != None:
        print(f"loading clip ckpt from {kwargs['clip_ckpt']}")
        clip_ckpt = CLIPModel.load_from_checkpoint(kwargs["clip_ckpt"], strict=False)
        model.root_module.load_state_dict(clip_ckpt.mol_encoder.state_dict(), strict=True)
        # model.gnn.load_state_dict(clip_ckpt.mol_encoder.state_dict(), strict=True)
    ...
    if kwargs["num_gpu"] > 1:
        ...
        if torch.distributed.get_rank() == 0:
            program_backup = 'src/ms_pred/mabnet'
            shutil.copytree(program_backup, os.path.join(tb_path, "backup"), dirs_exist_ok=True)
